...
function Home() {
    const [symptoms, setSymptons] = useState(''); // Changed to empty string
    ...
    useEffect(() => {
        const fetchData = async () => {
            if (!symptoms) return; // Prevent fetch if symptoms is empty
            ...
            try {
                const options = {
                    ...
                    body: JSON.stringify({ symptoms }), // Keep the variable name consistent
                };

                const response = await fetch("http://localhost:3001", options);
                console.log('Selected symptom:', symptoms);
    ...
    return (
        <article className='home-container'>
            ...
            <section className="consent-container">
                ...
                <select className="symptons-container" onChange={onHandleChange} value={symptoms}>
                ...
                {data !== null ? (
                    <div className={symptoms === '' ? 'data-container' : 'show-data'}>
